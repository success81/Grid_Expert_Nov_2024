...
Make connections subtle and innovative."""
            ...
            # Generate lists with error handling
            try:
...
Make each question specific, clear, and directly relevant to solving the problem."""
        ...
        for line in lines:
            line = line.strip()
            if line and any(line.startswith(str(i) + '-') for i in range(1, 5)):
...
def generate_advice():
    try:
        logger.info("Generating advice - request received")
        data = request.json
        main_question = data['question']
        mode = data['mode']
        additional_info = data.get('additional_info', '')
        ...
        # Generate expert lists here instead of during question generation
        logger.info(f"Generating expert lists for mode: {mode}")
        ...
        # Select experts
        if custom_experts:
            selected_experts = custom_experts[:num_experts]
        else:
            selected_experts = expert_manager.get_experts(mode, num_experts)

        if not selected_experts:
            logger.warning("No experts selected, using fallback experts")
            selected_experts = [
        ...
        logger.info(f"Selected {len(selected_experts)} experts")

        # Get simulation messages for frontend
        simulation_messages = []
...
Start with a creative, relevant title formatted in markdown."""
        ...
        # Generate individual expert analyses
        logger.info("Generating expert analyses")
        expert_analyses = []
        for i, expert in enumerate(selected_experts, 1):
            logger.info(f"Generating analysis for expert {i}")
...
- Key principles and patterns you observe
- Specific insights from your domain

## Expert Recommendations
- Concrete, actionable solutions
- Implementation guidance
- Tools and techniques from your field
- Innovative applications of your expertise
...
Make all recommendations specific to your expertise.
Use terminology and frameworks from your field.
Provide practical, actionable advice."""
        ...
        conclusion_prompt = f"""Based on the expert analyses for this question about {main_question}, create a powerful conclusion that:

First Paragraph:
- Synthesizes key insights from all experts
- Highlights crucial recommendations
- Shows how different perspectives complement each other
- Emphasizes the comprehensive nature of the solution

Second Paragraph:
- Provides clear, prioritized next steps
- Offers a compelling vision of successful implementation
- Addresses potential challenges holistically
